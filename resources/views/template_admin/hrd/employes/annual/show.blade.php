                    ...
                    <table class="table table-condensed table-borderless">
                        ...
                        @if ($employee->role_annual)
                            <tfoot>
                                <tr>
                                    <td>Annual</td>
                                    <th>: {{ $monthComming }} <sup class="{{ $supClass }}">
                                            ({{ $result }})</sup>
                                    </th>
                                    <td>Exdo</td>
...
<div class="modal-footer">
    {{-- <button type="button" class="btn btn-sm btn-secondary btn-rounded"
        data-bs-role="{{ route('annualeave.edit', $employee->nik) }}" id="modalAnnual"><i
            class="fas fa-calendar-alt"></i> Annual</button>
    <form action="{{ route('employes.destroy', $employee->id) }}" method="post">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-sm {{ $btn }} btn-rounded" id="modalDelete"><i
                class="{{ $class }}"></i> {{ $st }}</button>
    </form> --}}
    <button type="button" class="btn btn-sm btn-rounded btn-warning" id="modalEdit"
        data-bs-role="{{ route('annualeave.edit', $employee->id) }}"><i class="fas fa-edit"></i> Show</button>
